...
import { useGetDashboardStats, useGetMembershipReport } from "@workspace/api-client-react";
...
export default function Growth() {
  ...
  // Derive auto inputs from real data
  ...
  return (
    <div className="container mx-auto p-4 max-w-7xl pb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        ...
        <KpiCard
          title="Operational Cap"
          value={operationalCapacity}
          unit="members"
          icon={Target}
          note="85% fill"
          highlight={currentMembers >= operationalCapacity * 0.9}
        />
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={projection} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                ...
                <ReferenceLine y={operationalCapacity} stroke="hsl(var(--primary))" strokeDasharray="6 3" label={{ value: `Op. Cap ${operationalCapacity}`, fill: "hsl(var(--primary))", fontSize: 10, fontFamily: "monospace" }} />
                <ReferenceLine y={maxCapacity} stroke="hsl(var(--destructive))" strokeDasharray="6 3" label={{ value: `Max ${maxCapacity}`, fill: "hsl(var(--destructive))", fontSize: 10, fontFamily: "monospace" }} />
                <Line type="monotone" dataKey="members" name="Projected Members" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3, fill: "hsl(var(--primary))" }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </CardContent>
      ...
      <Card className="bg-card/50 border-border/50">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={projection} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fontFamily: "monospace", fill: "hsl(var(--muted-foreground))" }} />
                <YAxis tickFormatter={v => `£${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11, fontFamily: "monospace", fill: "hsl(var(--muted-foreground))" }} />
                <RechartsTooltip content={<CustomTooltip />} />
                <ReferenceLine y={monthlyRevenuePotential} stroke="hsl(var(--primary))" strokeDasharray="6 3" label={{ value: `Potential ${fmt(monthlyRevenuePotential)}`, fill: "hsl(var(--primary))", fontSize: 10, fontFamily: "monospace" }} />
                <Bar dataKey="revenue" name="Monthly Revenue" fill="hsl(var(--primary))" opacity={0.8} radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </CardContent>
